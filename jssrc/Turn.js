...
var Turn = BackBone.Model.extend({
    initialize: function(options) {
        ...
        this.getDie = function(scrambler) {
            if (this.cup.isEmpty()) {
                this.overflowBrains += this.count(new ZombieValue().values.BRAIN);
                this.refillCup(scrambler);
            }
            return this.cup.pickOne();
        };
        this.refillCup = function(scrambler) {
            var brains = this.piles[new ZombieValue().values.BRAIN].pickAll();
        ...
        this.shoot = function() {
            return this.count(new ZombieValue().values.SHOOTGUN) >= 3;
        };
    ...
    score: function() {
        ...
        return this.count(new ZombieValue().values.BRAIN) + this.overflowBrains;
    },
    itsOver: function() {
