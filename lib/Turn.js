...
var Turn = BackBone.Model.extend({
    ...
    status: function() {
        return {
            ...
            brains: this.piles[ZombieValue.values.BRAIN].list.map(this.getColor),
            footprints: this.piles[ZombieValue.values.FOOTPRINTS].list.map(this.getColor),
            shotguns: this.piles[ZombieValue.values.SHOTGUN].list.map(this.getColor)
        };    
    }
});
module.exports = Turn;